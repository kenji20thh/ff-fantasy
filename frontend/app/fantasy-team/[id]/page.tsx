...
export default function FantasyTeamDetails() {
  ...
  const ownerName =
    fantasyTeam?.username || `Player #${fantasyTeam?.user_id}`
  ...
  async function loadDay(
    ...
    try {
      ...
      const loadedDayPlayers = playerResponses.flatMap(response =>
      ...
      const selectedPlayers = selection.player_ids
        .map(playerID =>
          loadedDayPlayers.find(player => player.id === playerID)
        )
        .filter(
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const firstOpenDay =
          loadedDays.find(day => !isDayLocked(day)) ??
        ...
        if (firstOpenDay) {
          ...
          const participatingTeamIDs =
            getParticipatingTeamIDs(firstOpenDay)
          ...
          const loadedDayPlayers = playerResponses.flatMap(
          ...
          const selection = fantasy.days?.find(
            item => item.day_id === firstOpenDay.id
          )
  ...
  async function changeDay(day: TournamentDay) {
    if (loadingDay || selectedDay?.id === day.id || !fantasyTeam) {
      return
    }

    setSelectedDay(day)

    await loadDay(day, fantasyTeam, teams)
  }
  ...
  function togglePlayer(player: Player) {
    ...
    if (isDayLocked(selectedDay)) {
      setMessage('This tournament day is locked.')
      return
    }
    ...
    if (alreadySelected) {
      setSelected(
        selected.filter(item => item.id !== player.id)
      )
  ...
  async function saveDay() {
    ...
    try {
      if (isDayLocked(selectedDay)) {
        throw Error('This tournament day is locked.')
      }
      ...
      const existingSelection = getDaySelection(selectedDay.id)

      const newSelection: FantasyDaySelection = {
        id: existingSelection?.id ?? 0,
        day_id: selectedDay.id,
      ...
      const previousDays = fantasyTeam.days ?? []
      ...
      setMessage(`${selectedDay.name} saved successfully.`)
    } catch (e) {
      setMessage(errorMessage(e))
    } finally {
      setSaving(false)
  ...
  const selectedDayLocked = selectedDay
    ? isDayLocked(selectedDay)
    : false

  const canEdit = isOwner && !selectedDayLocked

  const participatingTeamIDs = selectedDay
    ? getParticipatingTeamIDs(selectedDay)
    : new Set<number>()

  const visiblePlayers = dayPlayers.filter(
    player =>
      selectedTeam === null || player.team_id === selectedTeam
  )

  const backHref = isOwner ? '/fantasy-team' : '/leaderboard'
  ...
  return (
    <main className="mx-auto min-h-screen max-w-6xl px-5 py-10">
      <Link href={backHref} className="eyebrow">
        {backLabel}
      </Link>
      ...
      <div className="mt-8 flex flex-wrap items-end justify-between gap-6">
        <div>
          ...
          <h1 className="section-title mt-2">
            {isOwner ? 'Your squad.' : `${ownerName}'s squad.`}
          </h1>
        </div>
        ...
        <>
          <section className="mt-8 border-b border-border pb-5">
            ...
            {isOwner && selectedDayLocked && (
              <div className="mt-5 border border-border bg-card p-4">
                <p className="font-bold uppercase">Day locked</p>

                <p className="mt-1 text-sm text-muted-foreground">
                  This day&apos;s selection can no longer be
                  changed.
                </p>
              </div>
            )}
              ...
              <div className="mt-5 border border-border bg-card p-6">
                <p className="text-sm text-muted-foreground">
                  {isOwner
                    ...
                    : `${ownerName} has no squad for this day.`}
                </p>
              </div>
            ) : (
          ...
          {isOwner && (
            <section className="mt-8 border-2 border-primary bg-card p-5">
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                Your captain receives the captain multiplier for
                this day.
              </p>
              ...
              {canEdit && (
                <button
                  ...
                  onClick={saveDay}
                  disabled={
                    saving || selected.length !== 4 || !captain
                  }
          ...
          {canEdit && (
            <section className="mt-10">
              ...
              {!loadingDay && visiblePlayers.length === 0 && (
                <p className="mt-8 text-sm text-muted-foreground">
                  No players are available for this tournament
                  day.
                </p>
              )}
            </section>
